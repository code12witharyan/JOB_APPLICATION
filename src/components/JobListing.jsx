            ...
            <div className='w-full lg:w-1/4 bg-white px-4'>
                {/* Search filter from hero component */}
                {
                    isSearched && (searchFilter.title !== "" || searchFilter.location !== "") && (<>
                        <h3 className='font-medium lext-lg mb-4'>
                            Current Search
                        </h3>
                        <div className=" mb-4 text-gray-600">
                            ...
                            {searchFilter.location && (<span className='inline-flex items-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded ml-2'>{searchFilter.location}
                ...
                <button className='bg-gray-400 border  px-4 py-2 rounded mb-6 lg:hidden' onClick={e => setShowFilter(prev => !prev)}>
                    ...
                    <ul>
                        {
                            JobCategories.map((category, index) => (
                                <li key={index} className='text-gray-600 py-2 px-3 hover:bg-gray-100 rounded cursor-pointer mt-2'>
                ...
                {filteredJobs.length > 0 && (
                    <div  className='flex items-center justify-center space-x-2 mt-10 '>
                        ...
                        {Array.from({ length: Math.ceil(filteredJobs.length / 6) }).map((_, index) => (
                            <a key={index} href="#job-list" ><button onClick={() => { setCurrentPage(index + 1) }} className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded ${currentPage === index + 1 ? 'bg-blue-100 text-blue-500' : 'text-gray-500'}`}>{index + 1}</button></a>
                        ))}
                        <a  href="#job-list">
