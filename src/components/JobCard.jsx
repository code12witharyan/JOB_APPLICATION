  ...
  return (
    <div className='border border-gray-200 p-6 shadow rounded  '>
      ...
      <div className='flex justify-between text-sm items-center mt-6'>
        <button onClick={()=>{navigate(`/apply-job/{job.id}`)}} className='bg-blue-600 text-white px-4 py-2 rounded '>Apply Now</button>
        <button onClick={()=>{navigate(`/apply-job/{job.id}`)}}className='border-gray-500 border text-gray-500 px-4 py-2 rounded '>Learn More</button>
      </div>
